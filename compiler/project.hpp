...
struct Manifest {
    ...
    std::string edition = ppstability::DEFAULT_EDITION;
    std::string language = ppstability::LANGUAGE_VERSION;
    int abi = ppstability::ABI_VERSION;
...
inline Manifest parse_manifest(const fs::path &root) {
    ...
    while (std::getline(input, raw)) {
        ...
        if (section == "package") {
            ...
            else if (key == "language") manifest.language = value;
            else if (key == "abi") {
                try { manifest.abi = std::stoi(value); }
                catch (...) { throw Error("error: Punpun.toml [package].abi must be an integer"); }
            }
    ...
    if (manifest.language != "1" && manifest.language != ppstability::LANGUAGE_VERSION)
        throw Error("error: package '" + manifest.name + "' requires unsupported PunPun language '" + manifest.language + "'; compiler supports 1.0");
    if (manifest.abi != ppstability::ABI_VERSION)
        throw Error("error: package '" + manifest.name + "' requires PunPun ABI " + std::to_string(manifest.abi) + "; compiler provides ABI " + std::to_string(ppstability::ABI_VERSION));
...
class Graph {
  public:
    ...
    std::string lockfile() const {
        ...
        out << "# Generated by PunPun " PP_VERSION ". Do not edit by hand.\n"
            << "version = " << ppstability::LOCKFILE_FORMAT << "\n"
            << "language = \"" << ppstability::LANGUAGE_VERSION << "\"\n"
            << "abi = " << ppstability::ABI_VERSION << "\n\n";
